...
class Individual:
    def __init__(self):
        self.distribution = np.random.rand(LETTERS)         
        self.fitness = 0
...
class Evolution: 
    ...
    def add_variation_init(self, distribution):
        ...
        for _ in range(bits_to_switch):
            ...
            distribution[index] = min(1, max(0, distribution[index] + random.uniform(-0.5,0.5)))
        return distribution
...
class Algorithm:
    ...
    def initial_analysis(self):
        # Plot the initial weights of letters a-z for generation 0, 5, 10, 15, 1
        ...
        for idx, generation_idx in enumerate(indexes):
            ...
            ax[idx].set_yticks(np.arange(0, 1, 1))
            ax[idx].set_xticklabels([chr(ord("a") + i) for i in range(LETTERS)])
            ax[idx].set_yticklabels(range(1, 1 + 1))
